...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("thresholds with 2/3 coins") {
    assert(totalCoinsThreshold(3)(2, List(1,2)))
    assert(moneyThreshold(3)(2, List(1,2)))
    assert(combinedThreshold(4, List(1,2,3))(2, List(1,2,3)))
    assert(!combinedThreshold(4, List(1,2,3))(1, List(1,2,3,4,5,6,7)))
  }
