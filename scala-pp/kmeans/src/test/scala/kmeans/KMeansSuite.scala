...
class KMeansSuite extends FunSuite {
  ...
  test("'update' should work for 'points' == GenSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) and 'means' == GenSeq((1, 0, 0), (-1, 0, 0))") {
    ...
    val means: GenSeq[Point] = IndexedSeq(mean1, mean2)
    val mean1Ex = new Point(1, 0, 0)
    val mean2Ex = new Point(-1, 0, 0)
    val meansEx: GenSeq[Point] = IndexedSeq(mean1Ex, mean2Ex)
    val classified = GenMap((mean1, GenSeq(p1, p2)), (mean2, GenSeq(p3, p4)))
    assert(update(classified, means) == meansEx)
  }

  test("'converge' should work for 'points' == GenSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) and 'means' == GenSeq((1, 0, 0), (-1, 0, 0))") {
    val mean1 = new Point(1, 1, 0)
    val mean2 = new Point(-1, 1, 0)
    val means: GenSeq[Point] = IndexedSeq(mean1, mean2)
    val mean1Ex = new Point(1, 0, 0)
    val mean2Ex = new Point(-1, 0, 0)
    val meansEx: GenSeq[Point] = IndexedSeq(mean1Ex, mean2Ex)
    assert(converged(1)(means, meansEx))
    assert(!converged(.9)(means, meansEx))
  }
