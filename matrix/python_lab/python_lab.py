...
three_digits_set = { a+b+c for a in {y*base**0 for y in digits} for b in  {y * base**1 for y in digits} for c in {y * base**2 for y in digits} }

## Task 10
S = {1, 2, 3, 4}
T = {3, 4, 5, 6}
S_intersect_T = { a for a in S for b in T if a == b }
...
LofL_sum = sum([sum(LofL[a]) for a in {0,1,2}])# use form: sum([sum(...) ... ])
